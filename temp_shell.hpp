...
namespace ShellKeskin
{
    class Shell
    {
        public:
            ...
            File * lsRecursive(); // #currentpathden pathVectorün son elemanına kadar(en uzunu) her pathe gelince elemanlarını printle (path ismiyle birlikte)
        ...
        private:
            vector<File *> filesystem; // #bunu private a yolla (dynamic_cast)
            vector<string> paths; // #Tüm pathler burada, sıralı şekilde dizilmeli(lengthine göre)
            vector<string> inputs;
            string currentPath; // #Anlık pathi tutar
